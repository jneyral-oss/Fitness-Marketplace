...
function App(){
  ...
  const showAlert = (msg, type='info') => { setAlert({ msg, type }); setTimeout(()=>setAlert(null), 3500); };
  ...
  async function registerTraining(planId){
  ...
  function logout(){ try{ localStorage.removeItem('fm_token'); localStorage.removeItem('fm_user'); }catch(e){} setToken(null); setCurrentUser(null); setStep(1); setPlans([]); showAlert('Sesión cerrada','info'); }
        ...
        <div className="space-y-4">
          <div>
            ...
            {['Perder Peso','Ganar Músculo','Mantenimiento'].map(g => (
              <button key={g} onClick={() => updateFormData('goal', g)} className={`w-full mb-2 p-3 border rounded ${formData.goal===g?'bg-blue-100':''}`}>{g}</button>
            ))}
          </div>
          <div>
            <p className="font-semibold mb-2">¿Cómo describirías tu nivel?</p>
            {['Principiante','Intermedio','Avanzado'].map(l => (
              <button key={l} onClick={() => updateFormData('level', l)} className={`w-full mb-2 p-3 border rounded ${formData.level===l?'bg-blue-100':''}`}>{l}</button>
            ))}
          </div>
          <div>
            <p className="font-semibold mb-2">¿Dónde entrenarás?</p>
            {['En Casa','En Gimnasio'].map(loc => (
              <button key={loc} onClick={() => updateFormData('location', loc)} className={`w-full mb-2 p-3 border rounded ${formData.location===loc?'bg-blue-100':''}`}>{loc}</button>
            ))}
          </div>
  ...
  const renderScreen = () => {
    switch(step){
      ...
      case 3: return <OnboardingForm />;
      case 4: return <HomeScreen />;
